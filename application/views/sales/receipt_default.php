...
<div id="receipt_wrapper" style="font-size:<?php echo $this->config->item('receipt_font_size');?>px">
	...
	<table id="receipt_items">
		...
		foreach($cart as $line=>$item)
		{
			...
			{
			?>
				<?php if(strlen(ucfirst($item['name'] . ' ' . $item['attribute_values'])) > 10){ ?>
				<tr><td colspan="<?php echo ($this->config->item('receipt_show_tax_ind') ) ? 5:4; ?>"><?php echo ucfirst($item['name'] . ' ' . $item['attribute_values']); ?></td></tr>
				<?php } ?>
				<tr>
					<?php 
						if(strlen(ucfirst($item['name'] . ' ' . $item['attribute_values'])) <= 10){
					?>
