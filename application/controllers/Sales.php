...
class Sales extends Secure_Controller
{
	...
	private function _load_customer_data($customer_id, &$data, $stats = FALSE)
	{
		...
		if($customer_id != -1)
		{
			...
			// echo json_encode($customer_info);die;
			$data['customer_id'] = $customer_id;
			if(!empty($customer_info->company_name))
			{
				// $data['customer'] = $customer_info->company_name;
				$data['customer'] = $customer_info->first_name.((strlen($customer_info->last_name) > 0) ? " ".$customer_info->last_name :'');
			}
			else
			{
	...
	private function _load_sale_data($sale_id)
	{
		...
		// echo json_encode($sale_info);die;
		$this->sale_lib->copy_entire_sale($sale_id);
		$data = array();
		$data['cart'] = $this->sale_lib->get_cart();
		// echo json_encode($data['cart']);die;
		$data['payments'] = $this->sale_lib->get_payments();
